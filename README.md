...
* [`src/cljs/fraudulent_checker/`](src/cljs/fraudulent_checker/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/fraudulent_checker/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `fraudulent-checker` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fraudulent_checker/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fraudulent-checker.example
  (:require [fraudulent-checker.config :as config])
